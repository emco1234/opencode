...
function runCodeSearch(p: ToolProps<Tool.Info>): ToolInline {
  const query = text(dict(p.input).query)
  return {
    icon: "◇",
...
function scrollCodeSearchStart(p: ToolProps<Tool.Info>): string {
  const query = text(dict(p.input).query)
  if (!query) {
...
function permCodeSearch(p: ToolPermissionProps<Tool.Info>): ToolPermissionInfo {
  const query = text(dict(p.input).query)
  return {
    icon: "◇",
    title: `Exa Code Search "${query}"`,
